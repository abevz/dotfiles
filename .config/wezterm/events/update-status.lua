...
wez.on("update-status", function(window, pane)
  ...
  local cwd = wez.nerdfonts.cod_location .. " " .. fun.get_cwd_hostname(pane, true)
  local workspace = wez.nerdfonts.md_fire .. " " .. wez.mux.get_active_workspace()
  ...
  for i, cell in ipairs { cwd, workspace, datetime } do
    local cell_bg = colors[i]
    local cell_fg = i == 1 and last_fg or colors[i - 1]
    local sep = icons.Separators.StatusBar.right
